<template>
    ...
    <div class="page-content-sub" v-if="jobs.length > 0" :class="['theme-transition', { 'dark': themeStore.darkMode }]">
        ...
        <span  v-if="jobs.length == 0" class="no-job-screen"><p>You have no declined jobs yet</p></span>
              ...
              components:{ JobCard, NavBar, ProfileNavBar, Footer, RouterLink, useRouter, LeftNav, PageFilter, Loader, Skeleton, DotLoader },
                ...
                methods: {
                    ...
                    fetchJobListings() {
                        ...
                        axios
                            .get(`${this.api_url}/user-completed-jobs`, {
                            headers: {
                                Authorization: `JWT ${token}`, // Add the token as an authorization header
                            },
                            })
                            .then((response) => {
                            this.jobs = response.data.completedJobs.reverse();
                            // if(this.jobs.length > 0){
                            //     this.jobs.reverse();
                            // };
                            console.log(this.jobs); // logs all jobs to the console to test for data type....
